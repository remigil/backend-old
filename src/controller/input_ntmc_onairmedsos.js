...
const Ntmc_onair_sosial = require("../model/ntmc_onair_mediasosial");
const { Op, Sequelize } = require("sequelize");
const _ = require("lodash");
const { AESDecrypt } = require("../lib/encryption");
const readXlsxFile = require("read-excel-file/node");
const fs = require("fs");
const Ntmc_onair_mediasosial = require("../model/ntmc_onair_mediasosial");
...
module.exports = class NtmcOnAirTVController {
    ...
    try {
      ...
      const getDataRules = {
        attributes: [
          ...
          [Sequelize.fn("sum", Sequelize.col("instagram")), "instagram"],
          [Sequelize.fn("sum", Sequelize.col("twitter")), "twitter"],
          [Sequelize.literal("SUM(facebook + instagram + twitter)"), "total"],
        ],
        where: wheres,
      };
      ...
      let rows = await Ntmc_onair_mediasosial.findAll(getDataRules);
      ...
      if (type === "day") {
        const asd = list_day.map((item, index) => {
          ...
          if (data) {
            finals.push({
              ...
              instagram: parseInt(data.dataValues.instagram),
        ...
        let abc = rows.map((element, index) => {
          return {
            ...
            instagram: parseInt(element.dataValues.instagram),
        ...
        const asd = list_month.map((item, index) => {
          ...
          if (data) {
            finals.push({
              ...
              instagram: parseInt(data.instagram),
  ...
  static add = async (req, res) => {
    ...
    try {
      ...
      let insertData = await Ntmc_onair_sosial.create(inputData, {
