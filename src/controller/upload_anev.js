...
const Laporan_upload_anev = require("../model/upload_anev");
...
module.exports = class LaporanUploadAnevController {
  static get = async (req, res) => {
    try {
      let { limit, page } = req.query;
      page = page ? parseInt(page) : 1;
      const resPage = pagination.getPagination(limit, page);
      const LaporanUploadAnev = await Laporan_upload_anev.findAndCountAll({
        order: [["date", "DESC"]],
        // raw: true,
        nest: true,
        limit: resPage.limit,
      ...
      let rows = LaporanUploadAnev.rows;
      ...
      response(res, true, "Succeed", {
        ...
        total_page: Math.ceil(
          parseInt(LaporanUploadAnev.count) / parseInt(resPage.limit)
        ),
        recordsFiltered: LaporanUploadAnev.count,
        recordsTotal: LaporanUploadAnev.count,
        datanya,
  ...
  static getWeb = async (req, res) => {
    try {
      ...
      const modelAttr = Object.keys(Laporan_upload_anev.getAttributes());
      ...
      const data = await Laporan_upload_anev.findAll(getDataRules);
      const count = await Laporan_upload_anev.count({
  ...
  static getId = async (req, res) => {
    try {
      ...
      const resPage = pagination.getPagination(limit, page);
      const Laporan_upload_anev = await Laporan_upload_anev.findAndCountAll({
      ...
      let rangeup =
        page + 1 <= Laporan_upload_anev.count
          ? page
          : Laporan_upload_anev.count;
      ...
      response(res, true, "Succeed", {
        ...
        rangeup,
        rangedown,
        total_page: Laporan_upload_anev.count,
        // total_page: Math.ceil(parseInt(Laporan_upload_anev.count) / parseInt(resPage.limit)),
        ...Laporan_upload_anev, // newsid,
      });
    } catch (e) {
      response(res, false, "Failed", e.message);
    }
  ...
  static getIdweb = async (req, res) => {
    try {
      const data = await Laporan_upload_anev.findOne({
        where: {
          // id: req.params.id,
  ...
  static add = async (req, res) => {
    ...
    try {
      ...
      Object.keys(fieldData).forEach((key) => {
        if (req.body[key]) {
          if (key == "file" || key == "cover") {
            ...
            fs.renameSync(
              path,
              "./public/uploads/Laporan_upload_anev/" + fileName,
              function (err) {
                if (err) throw err;
              }
            );
      ...
      let op = await Laporan_upload_anev.create(fieldValueData, {
  ...
  static delete = async (req, res) => {
    ...
    try {
      ...
      await Laporan_upload_anev.update(fieldValue, {
        where: {
  ...
  static hardDelete = async (req, res) => {
    const transaction = await db.transaction();
    try {
      await Laporan_upload_anev.destroy({
        where: {
          id: AESDecrypt(req.body.id, {
            isSafeUrl: true,
            parseMode: "string",
