...
Polda.hasMany(Input_operasi_langgarmobil, {
  foreignKey: "polda_id",
  as: "op_langgarmobil",
});
...
module.exports = class OperasiLanggarMobilController {
    ...
    try {
      ...
      let getDataRules = {
        ...
        attributes: [
          ...
          [Sequelize.fn("sum", Sequelize.col("tanpa_sabuk")), "tanpa_sabuk"],
          [Sequelize.fn("sum", Sequelize.col("lain_lain")), "lain_lain"],
          [
            Sequelize.literal(
              "SUM(lawan_arus + bermain_hp + pengaruh_alkohol + max_kecepatan + dibawah_umur + tanpa_sabuk + lain_lain)"
            ),
            "total",
          ],
        ],
        include: [
          {
            model: Input_operasi_langgarmobil,
            required: false,
            as: "op_langgarmobil",
            attributes: [],
            where: {
      ...
      finals.map((element, index) => {
        rows.push({
          ...
          tanpa_sabuk: parseInt(element.dataValues.tanpa_sabuk) || 0,
  ...
  static get_by_date = async (req, res) => {
    ...
    try {
      ...
      let rows = await Input_operasi_langgarmobil.findAll(getDataRules);
      ...
      if (type === "day") {
        const asd = list_day.map((item, index) => {
          ...
          if (data) {
            finals.push({
              ...
              tanpa_sabuk: parseInt(data.dataValues.tanpa_sabuk),
        ...
        let abc = rows.map((element, index) => {
          return {
            ...
            tanpa_sabuk: parseInt(element.dataValues.tanpa_sabuk),
        ...
        const asd = list_month.map((item, index) => {
          ...
          if (data) {
            finals.push({
              ...
              tanpa_sabuk: parseInt(data.tanpa_sabuk),
