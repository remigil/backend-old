...
const Ntmc_dok_mediatv = require("../model/ntmc_dok_mediatv");
...
module.exports = class NtmcOnAirTVController {
  static get_by_date = async (req, res) => {
    ...
    try {
      ...
      const getDataRules = {
        attributes: [
          [Sequelize.fn("sum", Sequelize.col("media_tv")), "media_tv"],
          [Sequelize.literal("SUM(media_tv)"), "total"],
        ],
        where: wheres,
      };
      ...
      let rows = await Ntmc_dok_mediatv.findAll(getDataRules);
      ...
      if (type === "day") {
        const asd = list_day.map((item, index) => {
          ...
          if (data) {
            finals.push({
              media_tv: parseInt(data.dataValues.media_tv),
        ...
        let abc = rows.map((element, index) => {
          return {
            media_tv: parseInt(element.dataValues.media_tv),
        ...
        const asd = list_month.map((item, index) => {
          ...
          if (data) {
            finals.push({
              media_tv: parseInt(data.media_tv),
  ...
  static add = async (req, res) => {
    ...
    try {
      let inputData = {
        media_tv: req.body.media_tv,
        date: req.body.date,
      };

      let insertData = await Ntmc_dok_mediatv.create(inputData, {
