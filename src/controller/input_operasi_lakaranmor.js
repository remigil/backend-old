...
Polda.hasMany(Input_operasi_lakaranmor, {
  foreignKey: "polda_id",
  as: "op_lakaranmor",
});
...
module.exports = class OperasiKendaraanController {
  static get = async (req, res) => {
    ...
    try {
      ...
      let getDataRules = {
        ...
        include: [
          {
            model: Input_operasi_lakaranmor,
            required: false,
            as: "op_lakaranmor",
            attributes: [],
            where: {
  ...
  static get_by_date = async (req, res) => {
    ...
    try {
      ...
      let rows = await Input_operasi_lakaranmor.findAll(getDataRules);
  ...
  //         });
  //       });
  //       let insertDataPolda = await Input_operasi_lakaranmor.bulkCreate(
  //         dataInputPolda,
  //         { transaction: transaction }
  //       );
  //     } else {
  //       let checkData = await Input_operasi_lakaranmor.findOne({
  //         where: {
  //           polda_id: decAes(req.body.polda_id),
  ...
  //       };
  //       if (checkData) {
  //         let updateData = await Input_operasi_lakaranmor.update(inputData, {
  //           where: {
  //             polda_id: decAes(req.body.polda_id),
  //             polres_id: decAes(req.body.polres_id),
  //             date: req.body.date,
  ...
  //         });
  //       } else {
  //         let insertData = await Input_operasi_lakaranmor.create(inputData, {
  //           transaction: transaction,
  //         });
  //       }
  //     }
