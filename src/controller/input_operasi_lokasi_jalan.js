...
Polda.hasMany(Input_operasi_lokasi_jalan, {
  foreignKey: "polda_id",
  as: "op_lokasi_jalan",
});
...
module.exports = class OperasiLokasiJalanController {
    ...
    try {
      ...
      let getDataRules = {
        ...
        attributes: [
          "id",
          "name_polda",
          [Sequelize.fn("sum", Sequelize.col("nasional")), "nasional"],
          [Sequelize.fn("sum", Sequelize.col("provinsi")), "provinsi"],
          [Sequelize.fn("sum", Sequelize.col("kab_kota")), "kab_kota"],
          [
            Sequelize.fn("sum", Sequelize.col("desa_lingkungan")),
            "desa_lingkungan",
          ],
          [
            Sequelize.literal(
              "SUM(nasional + provinsi + kab_kota + desa_lingkungan)"
            ),
            "total",
          ],
        ],
        include: [
          {
            model: Input_operasi_lokasi_jalan,
            required: false,
            as: "op_lokasi_jalan",
            attributes: [],
            where: {
      ...
      finals.map((element, index) => {
        rows.push({
          ...
          nasional: parseInt(element.dataValues.nasional) || 0,
          provinsi: parseInt(element.dataValues.provinsi) || 0,
          kab_kota: parseInt(element.dataValues.kab_kota) || 0,
          desa_lingkungan: parseInt(element.dataValues.desa_lingkungan) || 0,
  ...
  static get_by_date = async (req, res) => {
    ...
    try {
      ...
      let rows = await Input_operasi_lokasi_jalan.findAll(getDataRules);
      ...
      if (type === "day") {
        const asd = list_day.map((item, index) => {
          ...
          if (data) {
            finals.push({
              nasional: parseInt(data.dataValues.nasional),
              provinsi: parseInt(data.dataValues.provinsi),
              kab_kota: parseInt(data.dataValues.kab_kota),
              desa_lingkungan: parseInt(data.dataValues.desa_lingkungan),
        ...
        let abc = rows.map((element, index) => {
          return {
            nasional: parseInt(element.dataValues.nasional),
            provinsi: parseInt(element.dataValues.provinsi),
            kab_kota: parseInt(element.dataValues.kab_kota),
            desa_lingkungan: parseInt(element.dataValues.desa_lingkungan),
        ...
        const asd = list_month.map((item, index) => {
          ...
          if (data) {
            finals.push({
              nasional: parseInt(data.nasional),
              provinsi: parseInt(data.provinsi),
              kab_kota: parseInt(data.kab_kota),
              desa_lingkungan: parseInt(data.desa_lingkungan),
